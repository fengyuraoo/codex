...
DesignMate v0.6.1 的界面目标是让设计学生能在一个本地 Web UI 中完成“导入资料、搜索资料、整理分类、询问助手、查看报告、生成草稿、导出案例”的日常工作，同时能直接截图放入作品集展示。

## 顶部导航

- DesignMate v0.6.1 标识。
- “本地 AI 作品集资料库”定位。
- Dashboard / Search / Ask / Reports 四个主视图。
- 右下角状态显示 API 已连接、静态模式、保存中、保存成功或失败。
- Showcase Mode 按钮，用于隐藏部分开发提示，保留核心产品信息。
...
- Demo 数据、User Inbox、Library、Unknown Source 数量。
- 项目分布和类型分布。
- Ask DesignMate 入口。
- Portfolio Draft 入口。
- 高价值资料 Top 5 和最近更新资料。
...
- Source 筛选：All、Demo data、User inbox、Imported library、Unknown。
...
- 回答结构分为回答摘要、相关资料、设计判断、可以放进作品集的位置、需要确认、下一步建议。
- 显示 confidence 和当前 mode。
...
Drafts 和 Export 目前通过命令生成，并在 Reports 页面显示路径和命令提示：

- `scripts/generate_portfolio_draft.py` 生成页面草稿。
- `scripts/export_portfolio_case.py` 导出 DesignMate 作品集案例包。
- `drafts/draft_index.md` 列出已生成草稿。
- `portfolio_export/a3_portfolio_page_content.md` 和 `two_page_portfolio_story.md` 用于作品集排版。
